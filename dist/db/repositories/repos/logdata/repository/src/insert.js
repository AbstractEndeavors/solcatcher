...
import { firstRowIdOrNull, firstRowOrNull, LogDataRow } from '@imports';
import { expectSingleRow } from '@imports';
export async function insert(raw) {
    ...
    const result = await this.db.query(QueryRegistry.INSERT, values);
...
export async function insertBatch(rows) {
    ...
    const result = await this.db.query(QueryRegistry.BATCH_INSERT, values);
    return {
        ok: true,
        value: new Map(result.rows.map(r => [r.signature, r.id]))
    };
}
export async function insertUnknownInstruction(row) {
    await this.db.query(`
...
export async function insertIntent(signature) {
    const result = await this.db.query(QueryRegistry.INSERT_INTENT, [signature]);
    const id = firstRowIdOrNull(result);
    if (!id)
        throw new Error("insertIntent(): failed");
    return id;
}
