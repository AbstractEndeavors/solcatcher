...
import { firstRowIdOrNull, bindRepo } from '@imports';
...
export class MetaDataRepository {
    ...
    async markProcessed(mint) {
        const res = await this.db.query(QueryRegistry.MARK_PROCESSED, [mint]);
        if (!res.rows[0]?.id) {
...
export function createMetaDataRepository(db) {
