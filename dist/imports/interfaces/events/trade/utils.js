import { isDecodedResult, calculatePrecisePrice, lamportsToSol, toUiAmount, TransactionsInsertParams, isDbSafeInsertTransactionsParams, ensureString, isBool, isAddress, isId, } from './imports.js';
...
export function coerceTradeEventBigints(raw) {
    const out = { ...raw };
 ...
 */
export function isDecodedTradeEventData(d) {
    ...
    // ix_name enum
    if (obj.is_buy !== deriveIsBuy(obj.ix_name))
        return false;
    return true;
}
/**
 ...
 */
export function extractDecodedTradeEvent(raw) {
    if (!isTradeEvent(raw)) {
        return null;
    }
    if (!isDecodedTradeEventData(raw.data)) {
        console.warn('extractDecodedTradeEvent: data shape invalid', raw.data);
        return null;
    }
    return raw.data;
}
 ...
 * Returns null if not a valid TradeEvent.
 */
export function extractDecodedTradeEventErrorGuard(raw) {
    if (!isTradeEvent(raw)) {
 ...
 */
export function priceFromDecodedEvent(decoded) {
 ...
 */
export function enrichTradeEvent(decoded, ctx) {
 ...
 */
export function toInsertParams(enriched) {
...
export function toTransactionInsertParams(enriched) {
    const d = enriched.decoded;
    return new TransactionsInsertParams(enriched.log_id, enriched.pair_id, enriched.meta_id, enriched.signature, enriched.program_id, enriched.slot, enriched.invocation || enriched.invocation_index, d.mint, d.user_address, d.is_buy, d.ix_name, d.sol_amount, d.token_amount, d.virtual_sol_reserves, d.virtual_token_reserves, d.real_sol_reserves, d.real_token_reserves, d.mayhem_mode || enriched.mayhem_mode || false, enriched.price.float, d.track_volume || enriched.track_volume || false, d.total_unclaimed_tokens || enriched.total_unclaimed_tokens, d.total_claimed_tokens || enriched.total_claimed_tokens, d.current_sol_volume || enriched.current_sol_volume, d.fee_recipient || enriched.fee_recipient, d.fee_basis_points || enriched.fee_basis_points, d.fee || enriched.fee, d.creator, d.creator_fee_basis_points || enriched.creator_fee_basis_points, d.creator_fee || enriched.creator_fee, d.timestamp, d.last_update_timestamp);
}
// =============================================================================
// FULL PIPELINE
// =============================================================================
 ...
 */
export function preProcessTradeEvent(raw, ctx) {
    // Layer 1: Extract
    ...
    if (!isDbSafeInsertTransactionsParams(insertParams)) {
        throw new Error(`processTradeEvent: insertParams failed DB validation. ` +
 ...
 * Returns null if decode output is not a valid CreateEvent.
 */
export function processTradeEventErrorGuard(decoded, ctx) {
    if (!ctx.program_id) {
        throw new Error(`Missing program_id in TradePipelineContext`);
    }
    // Coerce the event itself, not just .raw — after JSON round-trip
    // through RabbitMQ, all bigint fields arrive as strings
    const safe = coerceTradeEventBigints(decoded);
    const tradeEvent = preProcessTradeEvent(safe, ctx);
    if (!tradeEvent) {
        let out = {};
        out.decoded = safe;
