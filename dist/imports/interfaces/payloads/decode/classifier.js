import { normalizeBigInt, extractDecodedTradeEventErrorGuard, extractDecodedCreateEventErrorGuard, isDecodedResult, } from './imports.js';
 ...
 */
export function classifyPayload(row, registry) {
    // ── gate: skip undecodable rows ──
    if (row.decodable === false)
        return null;
    if (!row.b64)
        return null;
    // ── decode ──
...
// ============================================================
export function classifyPayloadBatch(rows, registry) {
    const events = [];
    let skipped = 0;
...
// ============================================================
function buildTradeEvent(provenance, data) {
    return {
...
function buildCreateEvent(provenance, data) {
    return {
...
function buildUnknownEvent(provenance, discriminator, raw) {
    return {
