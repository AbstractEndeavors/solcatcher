...
// ============================================================
export function parseProgramLogs(logs) {
    const state = {
        stack: [],
        roots: [],
        invocationIndex: 0,
...
// ──────────────────────────────────────────────────────
function handleInvoke(match, state) {
...
function handleSuccess(match, state) {
    const program_id = match[1];
...
function handleLog(match, state) {
    const current = state.stack[state.stack.length - 1];
    if (current) {
        current.logs.push(match[1]);
    }
}
function handleData(match, state) {
    const current = state.stack[state.stack.length - 1];
    if (current) {
        current.data.push(match[1]);
    }
}
function handleCompute(match, state) {
    const program_id = match[1];
...
// ──────────────────────────────────────────────────────
function convertToImmutable(mut) {
    return new InvocationRecord(mut.program_id, mut.invocation_index, mut.depth, mut.logs, mut.data, mut.children.map(convertToImmutable), mut.reported_invocation, mut.compute);
}
