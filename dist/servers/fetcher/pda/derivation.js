 ...
 */
export function getMetadataPDAAddress(mint) {
 ...
 */
export function deriveBondingCurvePDA(mint, programId = PROGRAM_IDS.PUMP_FUN) {
    const mintParam = new MintParam(mint);
 ...
 */
export function getBondingCurvePDAAddress(mint, programId = PROGRAM_IDS.PUMP_FUN) {
 ...
 */
export function deriveAssociatedBondingCurvePDA(mint, bonding_curve, token_program = PROGRAM_IDS.TOKEN) {
    const mintParam = new MintParam(mint);
 ...
 */
export function getAssociatedBondingCurvePDAAddress(mint, bonding_curve, token_program = PROGRAM_IDS.TOKEN) {
 ...
 */
export function deriveAllPDAs(mint, program_id = PROGRAM_IDS.PUMP_FUN, token_program = PROGRAM_IDS.TOKEN) {
    ...
    const associated_bonding_urve = deriveAssociatedBondingCurvePDA(mint_param.pubkey, bonding_curve, token_program);
    return new DerivedPDAsResult(getPubkeyString(mint_param.address), getPubkeyString(metadata), getPubkeyString(bonding_curve), getPubkeyString(associated_bonding_urve), getPubkeyString(program_param.address), getPubkeyString(token_program));
}
// ═══════════════════════════════════════════════════════════
 ...
 */
export function isToken2022Mint(mintSuffix) {
 ...
 */
export function getTokenProgramForMint(mint) {
    const mint_param = new MintParam(mint);
    return isToken2022Mint(mint_param.address)
 ...
 */
export function deriveAllPDAsAuto(mint, program_id = PROGRAM_IDS.PUMP_FUN) {
    const token_program = getTokenProgramForMint(mint);
    const pdas = deriveAllPDAs(mint, program_id, token_program);
    return pdas;
}
