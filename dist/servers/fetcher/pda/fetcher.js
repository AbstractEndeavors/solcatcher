...
import { ensureString } from '@imports';
 ...
 */
export async function fetchSignatures(account, limit = 1000, before) {
    const res = await fetchSignaturesForAddress({ account, limit, before, commitment: 'confirmed' });
    if (!res?.result) {
        return [];
    }
    return res.result.map((sig) => ({
 ...
 */
export async function findOldestSignature(address) {
 ...
 */
export async function findFirstSignature(address) {
 ...
 */
export async function fetchRawTransaction(signature) {
 ...
 */
export function parseBondingCurveData(data) {
    ...
    return new BondingCurveDataResult(bs58.encode(data.subarray(8, 40)), // mint
 ...
 */
export async function fetchBondingCurveData(bondingCurveAddress) {
