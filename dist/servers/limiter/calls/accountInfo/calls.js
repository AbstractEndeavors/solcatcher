...
export async function getAccountInfoCalls(limiter, app) {
    app.post("/fetch-accountInfo", asyncHandler(async (req, res) => {
    ...
    // --------------------
    app.get("/fetch-accountInfoJsonParsed", asyncHandler(async (req, res) => {
        const input = readPublicInput(req);
        input.endoding = 'jsonParsed';
        res.json(await limiter.getAccountInfoJsonParsed(input));
    }));
    app.post("/fetch-accountInfoJsonParsed", asyncHandler(async (req, res) => {
        const input = readPublicInput(req);
        input.endoding = 'jsonParsed';
        res.json(await FetchAccountInfo(limiter, input));
    }));
