...
import { getSortedTxnHistory, getStringValue, getTimestamps, getTxnPrice, getSolAmount, getTokenAmount, getUserAddress, getMint, getBondingCurve, getSignature, getIsBuy, getVirtualSolReserves, getVirtualTokenReserves, ifListGetSingle, } from './../imports/index.js';
/* ---------------------------------------------
 * OHLC Resampling (pure function)
 * --------------------------------------------- */
export function resampleToOHLC(txns, intervalSeconds = 60) {
    if (txns.length === 0)
        return [];
 ...
 * --------------------------------------------- */
export function extractHighlights(txns, userAddress, intervalSeconds = 60) {
    return txns
 ...
 * --------------------------------------------- */
function resolvePairMeta(txn_history, pair_id, get_pair, get_creation_decoded_data) {
 ...
 * --------------------------------------------- */
export function processOHLC(params) {
    const { txn_history, interval_seconds = 60, highlight_user_address, pair_id, get_pair, get_creation_decoded_data, } = params;
    const sorted = getSortedTxnHistory(txn_history);
    ...
    const highlights = highlight_user_address
        ...
        : [];
    // Resolve metadata
    const { meta } = resolvePairMeta(txn_history, pair_id, get_pair, get_creation_decoded_data);
    return { ohlc, highlights, meta };
}
export function extractVolumeBars(ohlc, upColor = "#26a69a", downColor = "#ef5350") {
    return ohlc.map((bar) => ({
        time: bar.time,
        value: bar.volume,
