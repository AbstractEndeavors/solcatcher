...
export async function getPairCalls(app) {
    ...
    app.get("/pairs/cursor", async (req, res) => {
        try {
            ...
            const cursor = req.query.cursor_created_at && req.query.cursor_id
                ? {
                    created_at: new Date(String(req.query.cursor_created_at)),
                    id: Number(req.query.cursor_id),
                }
                : undefined;
            res.json(await pairsService.getCursorPage({
                limit,
                cursor,
            }));
        }
        ...
        try {
            const result = await ohlcHandler({
                ...
                query: {
                    ...
                    highlight_user: req.query.highlight_user,
                },
            });
            res.json(result);
        }
