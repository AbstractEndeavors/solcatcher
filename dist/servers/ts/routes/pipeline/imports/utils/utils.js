import { logs, MAX_LOGS, wsClients } from './../src/index.js';
// ────────────────────────────────────────────────────────
// STATE MANAGEMENT
// ────────────────────────────────────────────────────────
let pipelineState = null;
export function setPipelineState(state) {
    pipelineState = state;
}
export function getPipelineState() {
    return pipelineState;
}
export function requirePipeline(req, res, next) {
    if (!pipelineState) {
...
export function broadcastClearLogs() {
    broadcast({ type: 'logs_cleared', payload: null });
}
export function captureLog(entry) {
    const logEntry = {
