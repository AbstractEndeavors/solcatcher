...
export const QueryRegistry = {
  ...
  UPSERT_GENESIS: `
    ...
    ON CONFLICT (mint) DO UPDATE SET
      ...
      program_id = COALESCE(EXCLUDED.program_id, metadata.program_id),
      timestamp = COALESCE(EXCLUDED.timestamp, metadata.timestamp),
      metadata_pda = COALESCE(EXCLUDED.metadata_pda, metadata.metadata_pda),
      update_authority = COALESCE(EXCLUDED.update_authority, metadata.update_authority),
      mint_authority = COALESCE(EXCLUDED.mint_authority, metadata.mint_authority),
      freeze_authority = COALESCE(EXCLUDED.freeze_authority, metadata.freeze_authority),
      seller_fee_basis_points = COALESCE(EXCLUDED.seller_fee_basis_points, metadata.seller_fee_basis_points),
      is_mutable = COALESCE(EXCLUDED.is_mutable, metadata.is_mutable),
      primary_sale_happened = COALESCE(EXCLUDED.primary_sale_happened, metadata.primary_sale_happened),
      token_standard = COALESCE(EXCLUDED.token_standard, metadata.token_standard),
      image = COALESCE(EXCLUDED.image, metadata.image),
      description = COALESCE(EXCLUDED.description, metadata.description),
      external_url = COALESCE(EXCLUDED.external_url, metadata.external_url),
      onchain_metadata = COALESCE(metadata.onchain_metadata, '{}'::jsonb) || COALESCE(EXCLUDED.onchain_metadata, '{}'::jsonb),
      offchain_metadata = COALESCE(metadata.offchain_metadata, '{}'::jsonb) || COALESCE(EXCLUDED.offchain_metadata, '{}'::jsonb),
      spl_metadata = COALESCE(metadata.spl_metadata, '{}'::jsonb) || COALESCE(EXCLUDED.spl_metadata, '{}'::jsonb),
      status = CASE
        WHEN metadata.status = 'stub' THEN EXCLUDED.status
        ELSE metadata.status
      END,
      updated_at = NOW()
      -- claimed_at intentionally omitted: keep whatever is already set
    RETURNING id;
  `,
INSERT_IDENTITY:
    `
...
`,
  ...
  FETCH_BY_BONDING_CURVE: `SELECT * FROM metadata WHERE bonding_curve = $1 LIMIT 1;`,
  FETCH_BY_PROGRAM: `SELECT * FROM metadata WHERE program_id = $1;`,
...
RELEASE_SLOT: `
  UPDATE metadata SET claimed_at = NULL WHERE id = $1
`,

REAP_STALE: `
  UPDATE metadata SET claimed_at = NULL
