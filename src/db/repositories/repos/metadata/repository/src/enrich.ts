...
// ─────────────────────────────────────────────
  ...
  // ENRICH - Offchain
  // ─────────────────────────────────────────────
export async function upsertGenesis(
  this:MetaDataRepository,
    input: MetadataUpsertInput
  ): Promise<MetaDataRow> {
    if (!input.mint) {
...
export async function enrichOnchain(
  this: MetaDataRepository,
  id: IdLike,
  params: MetadataUpsertInput
): Promise<IdLike> {
  const res = await this.db.query<{ id: IdLike }>(
    QueryRegistry.ENRICH_ONCHAIN,
    [
      id,                                                                  // $1
      params.name             ?? null,                                     // $2
      params.uri              ?? null,                                     // $3
...
export async function enrichOffchain(
  this: MetaDataRepository,
  id: IdLike,
  params: OffchainMetadataPayload
): Promise<IdLike> {
  const offChain = params.offchain_metadata
  const res = await this.db.query<{ id: IdLike }>(
    QueryRegistry.ENRICH_OFFCHAIN,
    [
      id,                                                                        // $1
      params.image        ?? null,                                               // $2
