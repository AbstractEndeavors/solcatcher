...
export async function enrichPDAs(
  ctx: EnrichmentContext,
  _deps: any | AllDeps | null | EnrichmentDeps = null
...
export async function enrichGenesis(
  ctx: EnrichmentContext,
  deps: any | AllDeps | null | EnrichmentDeps = null
): Promise<EnrichmentContext> {
...
export async function enrichMetaDataLink(
  ctx: EnrichmentContext,
  deps: any | AllDeps | null | EnrichmentDeps = null
): Promise<EnrichmentContext> {
  deps = await getDeps(deps)
  if (ctx.meta_id || ctx.pair.meta_id) return ctx;

  const metaRow =
    ctx.meta || (await deps.metaDataRepository.fetchByMint(ctx.pair.mint));
...
export async function enrichEvents(
  ...
  deps: any | AllDeps | null | EnrichmentDeps = null
): Promise<EnrichmentContextWithEvents> {
  deps = await getDeps(deps)
  ctx = (await buildEnrichmentContext(
    deps,
    ctx,
    false
  )) as EnrichmentContextWithEvents;

  // ── gate: do we actually need event data? ──
  const needsReserves =
    ctx.pair.virtual_token_reserves == null ||
    ctx.pair.virtual_sol_reserves == null ||
    ctx.pair.real_token_reserves == null;

  const needsCreators =
    ctx.pair.creator == null ||
  ...
  const { trades, creates } = await resolveDecodedEvents(
    ctx,
    deps.logPayloads,
    ctx.pair.signature
  );
    ...
    const results = [
      {
        ...
        txn_id: null as IdLike,
      },
    ];
    const enrichmentTasks = [
      {
        queue: 'metaEnrich' as const,
        payload: {
          meta_id: ctx.meta.id || ctx.pair.meta_id,
          mint: trade.mint,
...
export async function enrichOnchainMeta(
  ctx: EnrichmentContext,
  deps: any | AllDeps | null | EnrichmentDeps = null
): Promise<EnrichmentContext> {
  deps = await getDeps(deps);
  ...
  if (!chainData?.metadata) return ctx;
  ...
  await deps.metaDataRepository.enrichOnchain(meta_id, {
...
export async function enrichOffchainMeta(
  ctx: EnrichmentContext,
  deps: any | AllDeps | null | EnrichmentDeps = null
): Promise<EnrichmentContext> {
  deps = await getDeps(deps)
  ...
  if (!offchain) return ctx;

  const meta_id = ctx.meta_id || getIdOrNull(ctx.meta) || ctx.pair.meta_id;

  await deps.metaDataRepository.enrichOffchain(meta_id, {
...
function applyCreateToContext(
  ...
  if (ctx.meta.offchain_metadata && ctx.meta.offchain_metadata?.description == null && create.description != null) {
  ...
  if (ctx.meta.user_address == null && create.raw.user != null) {
    ctx.meta.user_address = create.user;
