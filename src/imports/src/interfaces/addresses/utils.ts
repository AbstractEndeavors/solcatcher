...
export function normalizeAddress(input: any, label = "address"): string {
  if (!input) {
    return input
  ...
  throw new Error(
    `normalizeAddress(${label}): unsupported type ${typeof input}`
  );
}

export function isAddress(value: unknown): value is AddressLike {
