...
export function bigintToNumber(value: bigint): number {
  if (value > MAX_SAFE_INTEGER || value < -MAX_SAFE_INTEGER) {
    throw new RangeError(
      `bigintToNumber: value ${value} exceeds MAX_SAFE_INTEGER (${MAX_SAFE_INTEGER})`
    );
  }
  return Number(value);
}
 ...
 */
export function bigintToNumberClamped(value: bigint, context?: string): number {
  if (value > MAX_SAFE_INTEGER) {
    console.warn(
      `bigintToNumberClamped: ${context ?? 'value'} ${value} clamped to MAX_SAFE_INTEGER`
    );
...
export function normalizeBigInt(value: unknown): bigint {
