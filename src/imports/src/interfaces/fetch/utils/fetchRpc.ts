...
export function getFetchOptions(params:FetchOptions){
...
export async function fetchIt(params:FetchParams){
  const {url,
      method, 
      headers,
      body,
...
export async function fetchRpc(url:string,options: RpcParamsContext): Promise<any> {
  options.commitment = options.commitment || extractCommitment(options.params as any[]);
