import { initializeRegistry } from '@decoding';
import {LogPayloadInsert,ProgramDataEntry} from './../schemas.js'
import {partitionEvents} from '@imports';
...
export class DecodedLogs {
    private static _registry = initializeRegistry();
    ...
    getDecodedProgramData(REGISTRY: any=null): ProgramDataEntry[] {
...
export function decodeLogDataPayloads(logData:any){
 ...
 */
export function attachDecodedEvents(
  ctx: EnrichmentContext,
  ingestResult: IngestResult
): EnrichmentContextWithEvents {
  return {
    ...ctx,
 ...
 */
export async function resolveDecodedEvents(
  ctx: EnrichmentContextWithEvents,
  signature: SigLike
): Promise<PartitionedEvents> {
  ...
  const { trades, creates, unknowns } = await partitionEvents(ctx.decoded);
