...
export async function deriveBondingCurve(
  mint: MintLike,
  ...
  const [bondingCurvePk,bump] = await PublicKey.findProgramAddressSync(
    [Buffer.from("bonding-curve"), mintBytes],
    programPubkey
  );

const bonding_curve = stripQuotes(
  getPubkeyString(bondingCurvePk)
);
...
const [associatedBondingCurvePk] =
...
const associated_bonding_curve = stripQuotes(
  getPubkeyString(associatedBondingCurvePk)
);
 ...
 */
export async function deriveBondingCurves(
  mint: MintLike,
  programId: AddressLike
): Promise<{ bondingCurve: string; associatedBondingCurve: string }> {
  let mintPubkey = getPubkey(mint);
...
export async function bondingCurveSpec(
  ...
  if (
    ...
    bonding_curves.associated_bonding_curve =
      await deriveAssociatedBondingCurve(
        bonding_curves.bonding_curve,
        mint
      );
  }
