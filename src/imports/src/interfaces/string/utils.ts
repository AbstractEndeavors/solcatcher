...
export function normalizeString(
  ...
  if (typeof input === "string") {
    const v = input.trim();
    if (!v) throw new Error(`${field} is empty`);
    return v;
  }
...
export function isString(value: unknown,field:string): value is String {
...
export function ensureString(
  ...
  if (value == null) {
    ...
    throw new TypeError(
      `ensureString: ${opts?.label ?? 'value'} is null/undefined`
    );
  }
...
export function ensureStringOptional(value: unknown): string | null {
