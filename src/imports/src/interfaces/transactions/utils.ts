...
import {
  preProcessTradeEvent,
  type  IdLike,type AddressLike,type IntLike, type SigLike
} from './imports.js';
// =============================================================================
...
export function mapTradeEventToTransactionsInsert(params: {
  event: { name: string; category: string; data: Record<string, unknown> };
  signature: SigLike;
  slot: IntLike;
  ...
  if (!result) {
    throw new Error(
      `mapTradeEventToTransactionsInsert: not a valid TradeEvent. ` +
 ...
 */
export function isDbSafeInsertTransactionsParams(p: unknown): p is TransactionsInsertParams {
  ...
  for (const k of intIds) {
    if (!Number.isInteger(obj[k])) return false;
  }
  ...
  for (const k of requiredStrings) {
    if (typeof obj[k] !== 'string' || obj[k] === '') return false;
  }
  ...
  for (const k of numericStrings) {
    ...
    if (!/^-?\d+$/.test(val)) return false; // must be integer string
  }
  
  // boolean fields
  if (typeof obj.is_buy !== 'boolean') return false;
  if (typeof obj.mayhem_mode !== 'boolean') return false;
  if (typeof obj.track_volume !== 'boolean') return false;
  
  // ix_name enum
  if (obj.ix_name !== 'buy' && obj.ix_name !== 'sell') return false;
  
  // price (finite number)
  if (typeof obj.price !== 'number' || !Number.isFinite(obj.price)) return false;
  
  // timestamps (safe integers)
  if (!Number.isSafeInteger(obj.timestamp)) return false;
  if (!Number.isSafeInteger(obj.last_update_timestamp)) return false;
  
  return true;
}


export function toDbInsertParams(
  dto: TransactionsInsertDTO
): TransactionsInsertParams {
  return new TransactionsInsertParams(
    ...
    dto.sol_amount,
    dto.token_amount,
    dto.virtual_sol_reserves,
    dto.virtual_token_reserves,
    dto.real_sol_reserves,
    dto.real_token_reserves,
    dto.mayhem_mode,
    dto.price,
    dto.track_volume,
    dto.total_unclaimed_tokens,
    dto.total_claimed_tokens,
    dto.current_sol_volume,
    dto.fee_recipient,
    dto.fee_basis_points,
    dto.fee,
    dto.creator,
    dto.creator_fee_basis_points,
    dto.creator_fee,
    dto.timestamp,
    dto.last_update_timestamp
  );
}
