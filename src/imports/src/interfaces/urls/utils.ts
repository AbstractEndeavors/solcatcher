import type {UrlParse,UrlDict,UrlParsed} from './types.js';
export  { pathToFileURL } from "url";
export function parseUrl(
  input: string | { scheme: string; netloc: string }
): URL {
...
export function urlToString(url: UrlDict | string): string {
...
export function urlToDict(
  input: string | Partial<UrlDict>
): UrlDict {
  if (typeof input === "string") {
  ...
  if (!input.scheme || !input.netloc) {
    throw new Error(
      `[urlToDict] Invalid URL object: ${JSON.stringify(input)}`
    );
  }
...
export function normalizeUrlInput(input: UrlParsed): {
  identifier: string;
  scheme: string;
  netloc: string;
  url: string;
  ...
  if (!input.scheme || !input.netloc || !input.identifier) {
    throw new Error(
      `[RateLimiter] Invalid URL object: ${JSON.stringify(input)}`
    );
  }
...
export function getBaseDomain(
  input: string | { scheme: string; netloc: string } | UrlDict
): string {
  if (typeof input === "string") {
