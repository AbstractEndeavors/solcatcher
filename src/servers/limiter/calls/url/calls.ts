...
export async function getUrlCalls(limiter: any,app:any) {
 // --------------------
  // GET URL
  // --------------------
app.post("/get-url", asyncHandler(async (req:any, res:any) => {
  ...
  // GET FALLBACK
  // --------------------
  app.get("/get-fallback", asyncHandler( async (req:any, res:any) => {
    res.json(await limiter.getFallbackUrl());
  }));

  app.post("/get-fallback", asyncHandler(async (req:any, res:any) => {
    res.json(await limiter.getFallbackUrl());
  }));
  // --------------------
  // LOG RESPONSE (POST PRIMARY)
  // --------------------
  app.post("/log-response", asyncHandler( async (req:any, res:any) => {
    const input = readPublicInput(req);
    res.json(await limiter.logResponse(input));
  }));

  // GET allowed but discouraged
  app.get("/log-response", asyncHandler( async (req:any, res:any) => {
    const input = readPublicInput(req);
    res.json(await limiter.logResponse(input));
  }));
return app;
