...
import type {ChartRouteParams} from './imports/index.js';

export async function getChartCalls(app:any){
    // POST /api/charts/get-chart-data
    app.post("/charts/get-chart-data", async (req, res) => {
      try {
        const { id, interval, highlight_user }: ChartRouteParams = req.body;
  ...
  app.get("/pairs/:pair_id/ohlc", async (req, res) => {
    try {
      ...
      const result = await handler({
        ...
        query: req.query as { interval?: string; highlight_user?: string },
      });
